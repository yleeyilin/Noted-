...
function createCreateAndParams({ input, varName, node, context, callbackBucket, withVars, includeRelationshipValidation, topLevelNodeVariable, }) {
    ...
    if (node.auth) {
        ...
        if (authCypher) {
            ...
            creates.push(`CALL apoc.util.validate(NOT (${authCypher}), "${constants_1.AUTH_FORBIDDEN_ERROR}", [0])`);
            params = { ...params, ...authParams };
        }
    }
    if (meta?.authStrs.length) {
        creates.push(`WITH ${withVars.join(", ")}`);
        creates.push(`CALL apoc.util.validate(NOT (${meta.authStrs.join(" AND ")}), "${constants_1.AUTH_FORBIDDEN_ERROR}", [0])`);
    }
