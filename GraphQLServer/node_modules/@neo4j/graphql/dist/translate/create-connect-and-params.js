...
function createConnectAndParams({ withVars, value, varName, relationField, parentVar, refNodes, context, callbackBucket, labelOverride, parentNode, fromCreate, includeRelationshipValidation, isFirstLevel = true, }) {
    function createSubqueryContents(relatedNode, connect, index) {
        ...
        const preAuth = nodeMatrix.reduce((result, { node, name }) => {
            if (!node.auth) {
                return result;
            }
        ...
        if (preAuth.connects.length) {
            ...
            subquery.push(`\tCALL apoc.util.validate(NOT (${preAuth.connects.join(" AND ")}), "${constants_1.AUTH_FORBIDDEN_ERROR}", [0])`);
        ...
        */
        ...
        const postAuth = [...(!fromCreate ? [parentNode] : []), relatedNode].reduce((result, node) => {
            if (!node.auth) {
                return result;
            }
            const { cypher, params } = (0, create_auth_and_params_1.createAuthAndParams)({
        ...
        if (postAuth.connects.length) {
            subquery.push(`\tWITH ${[...withVars, nodeName].join(", ")}`);
            subquery.push(`\tCALL apoc.util.validate(NOT (${postAuth.connects.join(" AND ")}), "${constants_1.AUTH_FORBIDDEN_ERROR}", [0])`);
