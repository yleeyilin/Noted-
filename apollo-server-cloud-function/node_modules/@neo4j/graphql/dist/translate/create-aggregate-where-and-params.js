...
function aggregatePreComputedWhereFields({ value, relationField, relationship, context, matchNode, }) {
    ...
    const { returnProjections, predicates } = aggregateWhere(value, refNode, relationship, aggregationTarget, cypherRelation, context);
...
function aggregateWhere(aggregateWhereInput, refNode, relationship, aggregationTarget, cypherRelation, context) {
    const returnProjections = [];
    const returnPredicates = [];
    Object.entries(aggregateWhereInput).forEach(([key, value]) => {
        if (constants_1.AGGREGATION_AGGREGATE_COUNT_OPERATORS.includes(key)) {
            ...
            if (predicate)
                returnPredicates.push(predicate);
        }
            ...
            const { returnProjections: innerReturnProjections, predicates } = aggregateEntityWhere(value, refNodeOrRelation, target, context);
            ...
            (0, utils_2.asArray)(value).forEach((whereInput) => {
                const { returnProjections: innerReturnProjections, predicates } = aggregateWhere(whereInput, refNode, relationship, aggregationTarget, cypherRelation, context);
                returnProjections.push(...innerReturnProjections);
...
function aggregateEntityWhere(aggregateEntityWhereInput, refNodeOrRelation, target, context) {
    const returnProjections = [];
    const predicates = [];
    Object.entries(aggregateEntityWhereInput).forEach(([key, value]) => {
        if ((0, logical_operators_1.isLogicalOperator)(key)) {
            const logicalPredicates = [];
            (0, utils_2.asArray)(value).forEach((whereInput) => {
                const { returnProjections: innerReturnProjections, predicates: innerPredicates } = aggregateEntityWhere(whereInput, refNodeOrRelation, target, context);
                returnProjections.push(...innerReturnProjections);
