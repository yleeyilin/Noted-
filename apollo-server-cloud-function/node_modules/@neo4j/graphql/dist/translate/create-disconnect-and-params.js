    ...
    function createSubqueryContents(relatedNode, disconnect, index) {
        ...
        if (authorizationBeforeAndParams) {
            ...
            if (preAuth.disconnects.length) {
                ...
                subquery.push(`CALL apoc.util.validate(NOT (${preAuth.disconnects.join(" AND ")}), "${constants_1.AUTH_FORBIDDEN_ERROR}", [0])`);
        ...
        if (authorizationAfterAndParams) {
            ...
            // TODO: Authorization - delete for 4.0.0
            const postAuth = [parentNode, relatedNode].reduce((result, node) => {
                if (!node.auth) {
                    return result;
                }
                const { cypher, params } = (0, create_auth_and_params_1.createAuthAndParams)({
            ...
            if (postAuth.disconnects.length) {
                subquery.push(`WITH ${[...withVars, variableName].join(", ")}`);
                subquery.push(`CALL apoc.util.validate(NOT (${postAuth.disconnects.join(" AND ")}), "${constants_1.AUTH_FORBIDDEN_ERROR}", [0])`);
