...
const parse_value_node_1 = __importDefault(require("../schema-model/parser/parse-value-node"));
...
function getObjFieldMeta({ obj, objects, interfaces, scalars, unions, enums, callbacks, customResolvers, validateResolvers, }) {
    ...
    return obj?.fields?.reduce((res, field) => {
        ...
        if (relationshipMeta) {
            ...
            if (typeMeta.originalType?.kind === graphql_1.Kind.NON_NULL_TYPE) {
                if (typeMeta.originalType?.type.kind === graphql_1.Kind.LIST_TYPE) {
                    if (typeMeta.originalType?.type.type.kind !== graphql_1.Kind.NON_NULL_TYPE) {
                        throw new Error(msg);
                    }
                }
            }
            else if (typeMeta.originalType?.kind === graphql_1.Kind.LIST_TYPE) {
                throw new Error(msg);
            }
            const relationField = {
                ...baseField,
