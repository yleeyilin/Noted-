...
function buildRelationshipDestinationInterfaceNodeType({ relevantInterface, interfaceNodes, relationNodeTypeName, schemaComposer, }) {
    ...
    const [interfaceComposeFields, interfaceConnectionComposeFields] = [selectedFields, connectionFields].map(to_compose_1.objectFieldsToComposeFields);
...
function buildRelationshipDestinationAbstractType({ relationField, relationNodeTypeName, interfaceCommonFields, schemaComposer, nodeNameToEventPayloadTypes, }) {
    ...
    if (unionNodeTypes) {
        const unionNodes = (0, utils_1.filterTruthy)(unionNodeTypes?.map((typeName) => nodeNameToEventPayloadTypes[typeName]));
    ...
    if (interfaceNodeTypeNames) {
        ...
        const interfaceNodes = (0, utils_1.filterTruthy)(interfaceNodeTypeNames.map((name) => nodeNameToEventPayloadTypes[name]));
