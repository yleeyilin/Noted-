...
function filterDocument(document, features) {
    ...
    const filterFields = (fields, features) => {
        return fields
            ?.filter((field) => {
            ...
            .map((field) => {
            if (field.directives?.some((directive) => ["authentication", "authorization", "subscriptionsAuthorization"].includes(directive.name.value)) &&
                !features?.authorization) {
                console.warn("@authentication and/or @authorization detected - please ensure that you either specify authorization settings in features.authorization, or pass a decoded JWT into context.jwt on each request");
            }
            return {
                ...field,
                arguments: filterInputTypes(field.arguments),
                directives: field.directives?.filter((directive) => !["auth", "authentication", "authorization", "subscriptionsAuthorization"].includes(directive.name.value)),
            };
        });
    };
    return {
        ...
        definitions: document.definitions.reduce((res, def) => {
            ...
            if (def.kind === graphql_1.Kind.OBJECT_TYPE_DEFINITION || def.kind === graphql_1.Kind.INTERFACE_TYPE_DEFINITION) {
                ...
                if (def.directives?.some((x) => ["authentication", "authorization"].includes(x.name.value)) &&
                    !features?.authorization) {
                    console.warn("@authentication and/or @authorization detected - please ensure that you either specify authorization settings in features.authorization, or pass a decoded JWT into context.jwt on each request");
                }
                return [
                    ...
                    {
                        ...def,
                        directives: def.directives?.filter((x) => !["auth", "authentication", "authorization", "subscriptionsAuthorization"].includes(x.name.value)),
                        fields,
                    },
                ];
            }
            if (def.kind === graphql_1.Kind.SCHEMA_EXTENSION) {
                if (def.directives?.some((x) => ["authentication"].includes(x.name.value)) &&
                    !features?.authorization) {
                    console.warn("@authentication and/or @authorization detected - please ensure that you either specify authorization settings in features.authorization, or pass a decoded JWT into context.jwt on each request");
                }
                return [
...
function validateDocument({ document, features, additionalDirectives = [], additionalTypes = [], }) {
    const doc = filterDocument(document, features);
    const schemaToExtend = new graphql_1.GraphQLSchema({
        directives: [...Object.values(directives), ...graphql_1.specifiedDirectives, ...additionalDirectives],
        types: [
